...
export default function Editcustomer(props) {
    ...
    const handleSubmit = (editedCustomer) => {
        props.updateCustomer(editedCustomer, props.customer.links[0].href);
        handleCloseModal();
    }
    ...
    const modalBody = (
        <Formik 
        initialValues={{ firstname: props.customer.firstname, lastname: props.customer.lastname, streetaddress: props.customer.streetaddress, postcode: props.customer.postcode, city: props.customer.city, email: props.customer.email, phone: props.customer.phone }} 
    ...
    return(
        <div>
            <FontAwesomeIcon style={{cursor: 'pointer', display: 'inline-block'}} icon={faEdit} onClick={() => handleShowModal()} />
            <Modalcomponent show={showModal} modalBody={modalBody} handleClose={handleCloseModal} title='Edit customer details' /> 
        </div>        
    )
